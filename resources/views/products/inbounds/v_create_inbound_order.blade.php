...
@section('content')
                                            ...
                                            <td class="px-6">
                                                <div class=" w-[3rem] h-[2rem] border">
                                                    <img class="object-cover w-full h-full"
                                                        src={{ $master_product->mas_prod_image }} alt="">
                                                </div>
                                            </td>
    ...
    <script>
        ...
        const search_product = async () => {
            try {
                ...
                const cluster = '{{ env('CLUSTER') }}';
                //ส่ง req
                const response = await fetch(`${cluster}/product/inbounds/create-inbound-order/search-product`, {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': '{{ csrf_token() }}'
                ...
                if (response.ok) {
                    ...
                    const search_product_table = document.getElementById("lot_in_product_table");
        ...
        const add_product_cart_search = (mas_prod_id, cat_id) => {
            try {
                ...
                if (master_prod.length > 0) {
                    ...
                    let products_in_cart_localstorage = JSON.parse(localStorage.getItem('products_cart') || '[]');
                    products_in_cart_localstorage.push(newProduct);
                        ...
                        if (result.isConfirmed) {
                            if (!isNaN(quantity) && !parseInt(quantity) >= 0) {
                                if (document.getElementById('date').value === "") {
                                    ...
                                    const exp = document.querySelector(`#date`).value;
                                    newProduct.exp = exp;
                                    localStorage.setItem('products_cart', JSON.stringify(
                                        products_in_cart_localstorage));
                                    refresh_cart_table();
        ...
        //     console.log(cat_id)
        //     let master_prod = searches.filter((prod) => prod.mas_prod_id === mas_prod_id);
        //     let cat_prod = categories.filter((cats) => cats.cat_id === cat_id);
        //     if(master_prod.length>0){
        //         const quantity = document.getElementById(`quantity_search${master_prod[0].mas_prod_id}`).value;
        //         let newProduct = {
        //             ...master_prod[0], // Spread operator to copy all properties from master_prod
        //             amount: quantity, // Set the 'amount' property to the new quantity
        //             categories: cat_prod
        //         };

        //     }
        //     const product = JSON.parse(master_prod[0])
        //     console.log(product)
        //     // add quantity to products
        //     if (!isNaN(quantity) && !parseInt(quantity) >= 0) {
        //         product['amount'] = parseInt(quantity);
    ...
    //         </div>`,
        ...
        //                 categories: cat_prod
        //             };
        //             master_prod[0] = newProduct;
        //             console.log(master_prod[0])
        //             console.log(master_prod[0].categories.cat_name)
    ...
    //                 <div calss="flex flex-row gap-1">
    //                    <div class="flex gap-2">
    //                         <p class="font-bold">ชื่อ:</p><span>${master_prod[0].mas_prod_name}</span></br>
    //                    </div>
    //                     <div  class="flex gap-2">
    //                         <p class="font-bold">บาร์โค้ด:</p><span>${master_prod[0].mas_prod_barcode}</span>
    //                     </div>
    //                     <div  class="flex gap-2">
    //                         <p class="font-bold">จำนวน:</p><span>${master_prod[0].amount}</span>
    //                     </div>
    //                 </div>
    //                 <input id="date" type="date" class="swal2-input w-full cursor-pointer border input-primary py-2 px-2">
    //             </div>`,
        ...
        //             // add quantity to products
        //             if (!isNaN(quantity) && !parseInt(quantity) >= 0) {
        //                 master_prod['amount'] = parseInt(quantity);
        //                 products_in_cart_localstorage.push(master_prod);
        //             }
    ...
    //         </div>
    //         <div  class="flex gap-2">
    //             <p class="font-bold">ประเภท:</p><span>${master_prod[0].categories.cat_name}</span>
    //         </div>
    //         <div  class="flex gap-2">
    //             <p class="font-bold">จำนวน:</p><span>${master_prod[0].amount}</span>
    //         </div>
    ...
    // </div>`,
        ...
        //                             let newProductexp = {
        //                                 ...master_prod[
        //                                     0], // Spread operator to copy all properties from master_prod
        //                                 exp: exp // Set the 'amount' property to the new quantity
        //                             };
        //                             master_prod[0] = newProductexp;
        //                             master_prod = JSON.stringify(master_prod[0])
        //                             localStorage.setItem('products_cart', JSON.stringify(
        //                                 products_in_cart_localstorage));
        //                             refresh_cart_table();
        //                         }
